...
slopes <- slopes[f[, .(account_id, training_plan_id, nvr, wm, rot, tang)], on = "account_id"]

magnitude_assess <- lm(Slopes ~ Intercept + nvr + wm + rot + tang, data = slopes) # this might have a different n, since you aren't including the cluster indies
options(scipen = 999)
summary(lm.beta::lm.beta(magnitude_assess))
